...
void updateTone(unsigned long now) {
  if (trial.trialIsRunning && (stimPairType == "CS" || stimPairType == "CS_US")) {
    //Turning CS on and off while correct trial type running
    unsigned long ToneStart = trial.trialStartMillis + trial.preCSdur;
    unsigned long ToneStop = ToneStart + trial.CSdur;
    if (!toneCS.isOnTone && now >= ToneStart && now <= ToneStop) {
      toneCS.isOnTone = true;
      trial.CSstartMillis = now;
      serialOut(now, "ToneCSon", trial.currentTrial);
      digitalWrite(toneCS.TonePin,HIGH);
      
    } else if (toneCS.isOnTone && now > ToneStop) {
      toneCS.isOnTone = false;
      serialOut(now, "ToneCSoff", trial.currentTrial);
      digitalWrite(toneCS.TonePin,LOW);
    }
  }
}
