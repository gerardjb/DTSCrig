...
struct trial
{
  ...
  int sessionPin;//pin for projecting session state
  //CS and US
...
struct rotaryencoder
{
  ...
  //reading encoder
  unsigned long Rtime = 0; // keeps current time at milli precision to poll once per milli
  ...
  long printInterval = 2; //length of time between prints
  
  //Track CR motion
  boolean isOnCRcount = false; // on during CR detection
  long CRcount = 0; // tracks wheel motion during CR detection
  long CRthresh1 = -2; // sensitivity level for CR detection, lower number less sensitive
  long CRthresh2 = -13; // sensitivity level for the very small bop
  //Track ITI motion
...
struct twoP
{
  ...
  unsigned long fileChangeInt = 100; //counting until minimum inter-2P activation interval reached
...
struct toneCS
{
  boolean isOnTone; // use to toggle on and off
  int TonePin; // pin for the CS
};
...
struct Opto_US
{
  boolean isOnOpto_US = false;// use to toggel on and off DTSC trigger signal
  int OptoUSin;// pin for the US
};
...
typedef struct toneCS ToneCS;
typedef struct DTSC_US dtsc_US;
typedef struct Opto_US opto_US;
...
{ 
  ...
  trial.sessionPin = 12;//pin for conveying trial state
  trial.pinOnOff = false;//trial didn't just end
  pinMode(trial.sessionPin, OUTPUT);
  digitalWrite(trial.sessionPin, LOW);
  ...
  toneCS.TonePin = 9;
  toneCS.isOnTone = false;
  pinMode(toneCS.TonePin, OUTPUT);
  digitalWrite(toneCS.TonePin,LOW);
  ...
  Opto_US.OptoUSin = 11;
  //Opto_US.isOnOpto_US = false; 
  pinMode(Opto_US.OptoUSin,OUTPUT);
  digitalWrite(Opto_US.OptoUSin,LOW);
...
void startSession(unsigned long now) {
  if (trial.trialIsRunning==false) {
    ...
    digitalWrite(trial.sessionPin,HIGH);
...
void updateEncoder(unsigned long now) {
  ...
  if (rotaryencoder.Rtime<now&&trial.sessionIsRunning){
    rotaryencoder.Rtime = now;
    ...
    if(trial.trialIsRunning && now - rotaryencoder.printTime >= rotaryencoder.printInterval){
...
//Triggering the Tone CS
void updateTone(unsigned long now){
  if (trial.trialIsRunning && (trial.stimPairType=="CS"||trial.stimPairType=="CS_US")){
    
    if(trial.inCS && !toneCS.isOnTone){
      toneCS.isOnTone = true;
      serialOut(now,"toneCSon",trial.currentTrial);
      digitalWrite(toneCS.TonePin,HIGH);
    }else if(!trial.inCS && toneCS.isOnTone){
      toneCS.isOnTone = false;
      serialOut(now,"toneCoff",trial.currentTrial);
      digitalWrite(toneCS.TonePin,LOW);
    }
  }
}
...
void updateDTSC(unsigned long now){
  if (trial.trialIsRunning  && (trial.stimPairType=="US"||trial.stimPairType=="CS_US")){
    if(trial.inUS && !DTSC_US.isOnDTSC && DTSC_US.armedDTSC){
      ...
      digitalWrite(Opto_US.OptoUSin,HIGH);
    }
